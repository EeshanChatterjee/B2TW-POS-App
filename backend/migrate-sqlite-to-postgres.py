#!/usr/bin/env python3
"""
Migrate data from local SQLite database to remote PostgreSQL database
Usage: python3 migrate-sqlite-to-postgres.py
"""
...
def migrate_data():
    ...
    try:
        ...
        postgres_cursor.execute("ALTER TABLE customers ALTER COLUMN phone DROP NOT NULL")
        postgres_conn.commit()
        print("✅ Schema constraints fixed\n")
        ...
        migration_steps = [
            ...
            ("staff_users", migrate_staff_users),
...
def migrate_categories(sqlite_cursor, postgres_cursor, postgres_conn):
    """Migrate categories table"""
    print("🔄 Migrating categories...")

    sqlite_cursor.execute("SELECT id, name, position, created_at, updated_at FROM categories")
    ...
    for cat in categories:
        ...
        """, (cat['id'], cat['name'], cat['position'], cat['created_at'], cat['updated_at']))

    postgres_conn.commit()
    print(f"✅ Migrated {len(categories)} categories")


def migrate_products(sqlite_cursor, postgres_cursor, postgres_conn):
    ...
    for prod in products:
        postgres_cursor.execute("""
            ...
            ON CONFLICT (id) DO NOTHING
        """, (
            prod['id'], prod['name'], prod['category'], prod['price'], prod['description'],
            prod['position'], prod['veg_type'], prod['is_active'], prod['is_beverage'],
...
def migrate_customers(sqlite_cursor, postgres_cursor, postgres_conn):
    """Migrate customers table"""
    print("🔄 Migrating customers...")

    sqlite_cursor.execute("""
        SELECT id, phone, name, email, is_active, created_at, updated_at
        FROM customers
    """)
    ...
    for cust in customers:
        postgres_cursor.execute("""
            ...
            ON CONFLICT (id) DO NOTHING
        """, (
            cust['id'], cust['phone'], cust['name'], cust['email'],
            cust['is_active'], cust['created_at'], cust['updated_at']
        ))
...
def migrate_staff_users(sqlite_cursor, postgres_cursor, postgres_conn):
    """Migrate staff_users table"""
    print("🔄 Migrating staff users...")
    ...
    for user in users:
        ...
        """, (
            ...
            user['email'], user['role'], user['is_active'], user['created_at'], user['updated_at']
        ))
...
def migrate_orders(sqlite_cursor, postgres_cursor, postgres_conn):
    ...
    sqlite_cursor.execute("""
        SELECT id, customer_id, customer_phone, total_amount, status, payment_method, notes, created_at, updated_at
        FROM orders
    """)
    ...
    for order in orders:
        postgres_cursor.execute("""
            ...
            ON CONFLICT (id) DO NOTHING
        """, (
            order['id'], order['customer_id'], order['customer_phone'], order['total_amount'],
...
def migrate_bills(sqlite_cursor, postgres_cursor, postgres_conn):
    ...
    for bill in bills:
        ...
        """, (
            ...
            bill['customer_phone'], bill['total_amount'], bill['payment_method'], bill['status'],
...
def print_summary(sqlite_cursor, postgres_cursor):
    ...
    tables = [
        'categories', 'products', 'customers', 'staff_users',
        'orders', 'order_items', 'bills', 'kot_logs'
    ]
